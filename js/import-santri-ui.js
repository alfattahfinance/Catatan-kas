...
import { collection, getDocs, writeBatch, doc, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.17.1/firebase-firestore.js";
...
(() => {
  ...
  async function readFile(file) {
    ...
    if (!window.XLSX) await new Promise((resolve, reject) => {
      ...
      s.onload = resolve; s.onerror = () => reject(new Error('Library Excel gagal dimuat. Pastikan internet aktif.'));
  ...
  function toItems(rows) {
    ...
    const hasHeader = first.some(x => ['nama','namasantri','name','santri','kelas','class','wali','walisantri'].includes(x));
    const heads = hasHeader ? first : ['nama','kelas','wali'];
    const data = hasHeader ? rows.slice(1) : rows;
    const idx = (...aliases) => aliases.map(header).map(x => heads.indexOf(x)).find(i => i >= 0);
    const ni = idx('nama','namasantri','name','santri') ?? 0;
    const ki = idx('kelas','class') ?? 1;
    const wi = idx('wali','walisantri','orangtua') ?? 2;
    return data.map(r => ({ nama: String(r[ni] ?? '').trim(), kelas: String(r[ki] ?? '').trim() || '-', wali: String(r[wi] ?? '').trim() || '-' })).filter(x => x.nama);
  }

  async function importNow() {
    const file = $('fileImportSantri');
    ...
    try {
      const incoming = toItems(await readFile(file.files[0]));
      if (!incoming.length) throw new Error('Tidak ditemukan nama santri dalam file.');
      const snap = await getDocs(collection(db, 'santri'));
      const seen = new Set(snap.docs.map(d => norm(d.data()?.nama)).filter(Boolean));
      const unique = [];
      let skipped = 0;
      ...
      for (let i = 0; i < unique.length; i += 500) {
        ...
        unique.slice(i, i + 500).forEach(x => batch.set(doc(collection(db, 'santri')), { nama:x.nama, kelas:x.kelas, wali:x.wali, uid:auth.currentUser.uid, createdAt:serverTimestamp() }));
        await batch.commit();
      }
      status.textContent = `${unique.length} santri berhasil diimpor${skipped ? ` • ${skipped} dilewati` : ''}.`;
      alert(`${unique.length} santri berhasil diimpor.${skipped ? `\n${skipped} data dilewati karena sudah ada/duplikat.` : ''}`);
      file.value = '';
    } catch (e) {
      console.error(e); status.textContent = e.message || 'Import gagal.'; alert('Import gagal.\n\n' + (e.message || e));
    } finally { button.disabled = false; }
  }

  function install() {
    if ($('importSantriCard')) return;
    const table = $('tabelSantri');
    const listCard = table?.closest('.custom-card');
    if (!listCard?.parentElement) return;
    const card = document.createElement('div');
    card.id = 'importSantriCard'; card.className = 'card custom-card mb-4';
