...
(() => {
  ...
  const localNames=()=>{const set=new Set();try{const key=userAktif?.uid?`daftarSantri_${userAktif.uid}`:"daftarSantri";const list=JSON.parse(localStorage.getItem(key)||"[]");if(Array.isArray(list))list.forEach(x=>{const n=String(x?.nama||x?.namaSantri||x?.nama_santri||"").trim();if(n)set.add(n)})}catch(_){}return set};
  ...
  const start=()=>{ensureUI();rebuildLocal();if(unsubscribe){unsubscribe();unsubscribe=null}if(!userAktif){if(input?.value.trim())render();return}const q=query(collection(db,"santri"),where("uid","==",userAktif.uid));unsubscribe=onSnapshot(q,snap=>{const unique=new Set();snap.forEach(d=>{const x=d.data()||{},name=String(x.nama||x.namaSantri||x.nama_santri||"").trim();if(name)unique.add(name)});names=[...unique];rebuildLocal();if(input?.value.trim())render()},err=>{console.warn("Autocomplete peserta didik gagal memuat akun:",err);rebuildLocal();if(input?.value.trim())render()});if(input?.value.trim())render()};
  ...
  window.addEventListener("storage",e=>{if(e.key==="daftarSantri"||e.key?.startsWith("daftarSantri_")){rebuildLocal();if(input?.value.trim())render()}if(e.key==="pengaturanAplikasi")setTimeout(applyBoxTheme,0)});
