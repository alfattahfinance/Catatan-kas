...
(() => {
  ...
  const localNames = () => { const set=new Set(); try { const list=JSON.parse(localStorage.getItem("daftarSantri")||"[]"); if(Array.isArray(list))list.forEach(x=>{const n=String(x?.nama||x?.namaSantri||x?.nama_santri||"").trim();if(n)set.add(n);}); } catch(_){} return set; };
  ...
  const start = () => { ensureUI();rebuildLocal();if(unsubscribe)unsubscribe();unsubscribe=onSnapshot(collection(db,"santri"),snap=>{const unique=new Set();snap.forEach(d=>{const x=d.data()||{};const name=String(x.nama||x.namaSantri||x.nama_santri||"").trim();if(name)unique.add(name);});names=[...unique];rebuildLocal();if(input?.value.trim())render();},err=>{console.warn("Autocomplete peserta didik memakai daftar lokal:",err);rebuildLocal();if(input?.value.trim())render();});if(input?.value.trim())render(); };
  const startWhenReady=()=>{if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>setTimeout(start,0),{once:true});else setTimeout(start,0);};
  startWhenReady();
  document.addEventListener("click",e=>{if(box&&!box.contains(e.target)&&e.target!==input)hide();});
  window.addEventListener("storage",e=>{if(e.key==="daftarSantri"){rebuildLocal();if(input?.value.trim())render();}if(e.key==="pengaturanAplikasi")setTimeout(applyBoxTheme,0);});
