...
public class MainActivity extends AppCompatActivity {
    ...
    public class AndroidDownload {
        @JavascriptInterface
        public void downloadApk(String apkUrl, String version) {
    ...
    private void startApkDownload(String apkUrl, String version) {
        try {
            ...
            request.addRequestHeader("User-Agent", "Mozilla/5.0 Android CatatanKas");
            request.setTitle("Update Keuangan v" + cleanVersion);
            request.setDescription("Mengunduh aplikasi Keuangan v" + cleanVersion);
            ...
            if (directory != null) pendingApkFile = new File(directory, fileName);

            sendJavascript("window.dispatchEvent(new CustomEvent('apkDownloadStarted',{detail:{version:'"
                    + escapeJs(cleanVersion) + "'}}))");
            showMessage("Download update v" + cleanVersion + " dimulai...");
        } catch (Exception error) {
            error.printStackTrace();
            showMessage("Gagal memulai download APK.");
        }
    }
    ...
    private void checkDownloadResult(long completedId) {
        try {
            ...
            if (cursor == null || !cursor.moveToFirst()) {
                showMessage("Status download tidak ditemukan.");
                return;
            }
            ...
            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                ...
                if (pendingApkFile != null && pendingApkFile.exists()) {
                    ...
                    showMessage("APK berhasil diunduh, tetapi file tidak ditemukan.");
                }
                return;
            }

            if (status == DownloadManager.STATUS_FAILED) {
                sendJavascript("window.dispatchEvent(new CustomEvent('apkDownloadFailed'))");
                showMessage("Download APK gagal. Kode: " + reason);
            }
        } catch (Exception error) {
            error.printStackTrace();
            showMessage("Terjadi kesalahan saat memeriksa download.");
        }
    }

    private File resolveDownloadedFile(String localUri) {
        try {
    ...
    private void installApk(File apkFile) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
            ...
            try {
                ...
                error.printStackTrace();
                showMessage("Silakan aktifkan izin Install aplikasi tidak dikenal.");
            }
            return;
        }
    ...
    private void openApkInstaller(File apkFile) {
        try {
            ...
            error.printStackTrace();
            showMessage("Tidak dapat membuka installer APK.");
        }
    }

    private void deleteOldApk(String fileName) {
        try {
            File directory = getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS);
    ...
    private void openExternalUrl(Uri uri) {
        try {
            ...
            showMessage("Tidak dapat membuka tautan.");
        }
    }

    private String cleanVersion(String version) {
        if (version == null) return "";
        return version.replaceAll("[^0-9A-Za-z._-]", "").trim();
    }
