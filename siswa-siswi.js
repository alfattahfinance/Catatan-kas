...
async function load(){const el=$("daftarSantri"),tbody=$("tbodySantri");if(!el&&!tbody)return;if(unsubscribe){unsubscribe();unsubscribe=null}serverLoaded=false;if(!userAktif){renderList(readCache());return}const cached=readCache();if(cached.length)renderList(cached);else{if(el)el.innerHTML='<div class="text-center text-muted py-3">Memuat daftar nama...</div>';if(tbody)tbody.innerHTML='<tr><td colspan="4" class="text-center text-muted py-4">Memuat daftar nama...</td></tr>'}const q=query(collection(db,"santri"),where("uid","==",userAktif.uid));try{const first=await getDocs(q);if(auth.currentUser?.uid===userAktif.uid)renderServer(first);unsubscribe=onSnapshot(q,s=>{if(auth.currentUser?.uid===userAktif.uid)renderServer(s)},e=>{console.warn("Firestore daftar nama listener:",e);const c=readCache();renderList(c);})}catch(e){console.error(e);renderList(readCache())}}
window.simpanSantri=async()=>{if(!userAktif)return alert("Silakan login terlebih dahulu.");const nama=namaInput()?.value.trim()||"",kelas=kelasInput()?.value.trim()||"-",wali=waliInput()?.value.trim()||"-",id=idInput()?.value||"";if(!nama)return alert("Silakan isi nama lengkap.");try{if(id){const ref=doc(db,"santri",id),old=await getDoc(ref);if(!old.exists()||old.data()?.uid!==userAktif.uid)return alert("Data ini bukan milik akun Anda.");await updateDoc(ref,{nama,kelas,wali,updatedAt:serverTimestamp(),uid:userAktif.uid});}else{await addDoc(collection(db,"santri"),{nama,kelas,wali,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),uid:userAktif.uid,akunEmail:userAktif.email||""});}await load();kosongkan();refresh();alert(id?"Data berhasil diperbarui.":"✅ Nama berhasil ditambahkan.")}catch(e){alert("Gagal menyimpan data.\n\n"+(e.message||e))}};
