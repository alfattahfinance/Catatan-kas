...
function bacaSantriLokal(){try{const data=JSON.parse(localStorage.getItem("daftarSantri")||"[]");return Array.isArray(data)?data.map((x,i)=>({id:x?.id??`local-${i}`,...x,source:"local"})).filter(x=>String(x?.nama||x?.namaSantri||"").trim()):[];}catch(_){return[];}}
function gabungkanSantri(firestoreList,localList){const seen=new Set();const result=[];for(const item of [...firestoreList,...localList]){const nama=String(item?.nama||item?.namaSantri||item?.nama_santri||"").trim();const k=nama.toLocaleLowerCase("id-ID").replace(/\s+/g," ");if(!k||seen.has(k))continue;seen.add(k);result.push({...item,nama});}return result;}
function mulaiSantri(){if(unsubscribeSantri)unsubscribeSantri();unsubscribeSantri=onSnapshot(collection(db,"santri"),function(snapshot){const cloud=snapshot.docs.map(item=>({id:item.id,...item.data(),source:"firestore"}));semuaSantri=gabungkanSantri(cloud,bacaSantriLokal());tampilkanDashboard();},function(error){console.error("Gagal membaca santri dari Firestore:",error);semuaSantri=bacaSantriLokal();tampilkanDashboard();});}
...
window.addEventListener("storage",function(event){if(event.key==="catatanKasDataBerubah"||event.key==="logoDashboard"||event.key==="pengaturanAplikasi"||event.key==="jenisKeuanganCustom"||event.key==="daftarSantri"){mulaiSantri();refreshLogoDanDashboard();}});
window.addEventListener("daftarSantriBerubah",function(){mulaiSantri();refreshLogoDanDashboard();});
