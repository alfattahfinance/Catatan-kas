...
function renderServer(snap){const server=cleanList(snap.docs.map(d=>({id:d.id,...d.data()})));serverLoaded=true;
  if(server.length){writeCache(server);renderList(server)}else{const cached=readCache();renderList(cached)}
}
async function load(){const el=$("daftarSantri"),tbody=$("tbodySantri");if(!el&&!tbody)return;if(unsubscribe){unsubscribe();unsubscribe=null}serverLoaded=false;if(!userAktif){renderList(readCache());return}const cached=readCache();if(cached.length)renderList(cached);else{if(el)el.innerHTML='<div class="text-center text-muted py-3">Memuat data peserta didik...</div>';if(tbody)tbody.innerHTML='<tr><td colspan="4" class="text-center text-muted py-4">Memuat data peserta didik...</td></tr>'}
const q=query(collection(db,"santri"),where("uid","==",userAktif.uid));try{const first=await getDocs(q);if(auth.currentUser?.uid===userAktif.uid)renderServer(first);unsubscribe=onSnapshot(q,s=>{if(auth.currentUser?.uid===userAktif.uid)renderServer(s)},e=>{console.warn("Firestore data peserta listener:",e);const c=readCache();renderList(c);})}catch(e){console.error(e);renderList(readCache())}}
...
// Tampilan dibuat umum: tidak menampilkan istilah "Santri" kepada pengguna.
function gunakanIstilahUmum(){
  ...
  if(document.title)document.title=document.title.replace(/Santri/gi,"Peserta Didik");
  const meta=document.querySelector('meta[name="description"]');
  if(meta)meta.content=meta.content.replace(/Santri/gi,"Peserta Didik");
  ganti(document.body);
}
...
window.addEventListener("daftarSantriBerubah",()=>{if(userAktif)load()});import("./js/import-santri-ui.js").catch(()=>{});onAuthStateChanged(auth,u=>{userAktif=u||null;load()});